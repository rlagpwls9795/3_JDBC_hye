...
public class MemberView {
	...
	public void memberMenu(Member loginMember) {
		...
		this.loginMember= loginMember; //전달 받은 로그인 회원 정보를 필드에 저장
	...
	private void selectAll() { 
		...
		try {
			...
			List<Member> memberList = service.seletAll();
			
			//조회 결과가 있으면 모두 출력, 없으면 "조회 결과가 없습니다" 출력
	...
	private void updateMember() {
		try {
			...
			if(result>0) { //성공
				// loginMember에 저장된 값과 DB에 수정된 값을 동기화하는 작업이 필요
			...
			while(true) {
				...
				if(ch=='Y') {
					...
					if(result>0) {
						...
						//MainView의 loginMember를 null로 변경 (로그아웃)
