...
public class JDBCExample {
	public static void main(String[] args) {
		//JDBC : Java에서 DB에 연결(접근)할 수 있게 해주는 Java Programming API(Java에 기본 내장된 인터페이스)
		//java.sql 패키지에서 제공
		
		// * JDBC를 이용한 애플리케이션을 만들 때 필요한 것
		// 1. Java의 JDBC 관련 인터페이스 
		// 2. DBMS(Oracle)
		// 3. Oracle에서 Java 애플리케이션과 연결할 때 사용할 JDBC를 받아 구현한 클래스 모음(ojdbc11.jar 라이브러리)
		...
		try {
			...
			//-> DriverManager객체를 이용해서 Connection 객체를 만들어 얻어 옴
			...
			//3. SQL 작성
			// ** Java에서 작성되는 SQL은 마지막에 ;을 찍지 않아야 한다.
			...
			//5. 생성된 Statement 객체에 SQL을 적재하여 실행한 후 결과를 반환받아와 rs 변수에 저장
			rs=stmt.executeQuery(sql);
			//executeQuery() : SELECT문 실행 메서드, ResultSet 반환
			
			// 3단계 : SQL을 수행해서 반환받은 결과(ResultSet)를 한 행씩 접근해서 컬럼 값 얻어오기 
			while(rs.next()) {
				...
				String empName=rs.getString("EMP_NAME"); //현재 행의 "EMP_NAME" 문자열 컬럼의 값을 얻어 옴
				int salary=rs.getInt("SALARY"); //현재 행의 "SALARY" 숫자(정수) 컬럼의 값을 얻어 옴
